	...
	dfes = new FES({
		...
		"views":{
			...
			"LAD":{
				...
				"popup": {
					"text": function(attr){
						file = 'LAD-'+attr.properties.LAD23CD+'-'+this.options.scenario.replace(/ /,"").toLowerCase()+'-'+this.options.parameter+'.png';
						return '<h3>'+(attr.properties.LAD23NM || '?')+'</h3><p>'+attr.parameter.title+': '+attr.value.toLocaleString()+attr.parameter.units+' ('+this.options.key+')</p><div id="barchart">barchart</div><p class="footnote">The local authority may have been clipped to UKPN\'s area</p><p class="footnote capture-hide"><a href="#" onClick="saveDOMImage(document.querySelector(\'.dfes-popup-content\'),{\'src\':\''+file+'\',\'scale\':true});">Save chart as PNG</a></p>';
					},
					"open": function(attr){
